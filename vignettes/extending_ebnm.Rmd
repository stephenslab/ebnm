...
## The EBNM model for the scaled-*t* prior family
...
In general, fitting the EBNM model involves the following steps:

1. Define the prior family class.

2. Implement a function that estimates the prior.

3. Implement a function that computes posterior summaries.

4. Implement the main EBNM solver function.

5. Test the EBNM solver.

6. Use the new EBNM solver to analyze a data set.

<!-- Given observations $x_i$ with standard errors $s_i$, estimate $g
\in \mathcal{G}$; Given an estimate $\hat{g} \in \mathcal{G}$, compute
posterior means $\mathbb{E}(\theta_i \mid x_i, s_i, \hat{g})$ and/or
other quantities of interest. -->

In the following sections, we work through each of these steps in detail.

We then incorporate these implementations into a new
function `ebnm_t()` that uses the common interface supplied by the
**ebnm** package.

<!-- After checking our new function using the convenient checker
`ebnm_check_fn()`, we demonstrate that, as expected, our new function
performs better than the family of normal priors when the "true means"
$\theta_i$ are simulated from a scaled-*t* distribution. -->

The **ebnm** package is intended to be quite general and so should
permit implementation of a wide range of prior families. In terms of
implementation, we advise adhering to the
[Tidyverse style guide][tidyverse-style]. The functions
should also be carefully tested; at a minimum, functions should pass
...
`"normalmix"` class. Here, we define a new class.
...
this step, and the best approach very much depends on context. For an
...
function) to maximize the log likelihood. There are at least a couple of
...
both need to be positive. To avoid numerical issues, it can also be helpful to set
sensible upper and lower bounds. Here, we use the constraints 
...
Computing the log likelihood involves computing 1-d integrals, or
...
In some cases, providing the gradient calculations to "optim" can
greatly speed up the optimization. (This is what we found empirically
in our numerical experiments in [the paper][ebnm-paper].) For
implementing your own custom EBNM solver, you should consider
providing gradients, particularly when analytic expressions for the
gradients are available. (Or when automatic differentiation methods
are available.)

Gradients for the scaled-*t* priors turn out to be difficult to
obtain, but to illustrate how one might provide gradients to "optim",
here we compute gradients using the "grad" function from the **pracma**
package, which estimates the gradients numerically. This is not a
particularly efficient calculation so we do not expect this to lead to
faster optimization, but we include this code for illustrative purposes.

The new `opt_t` function has one change from the previous
definition—we add one line to specify the "gr" argument:
...
results to the implementation without gradients.
...
deviation. This is all accomplished inside a single function that returns a
...
This is most certainly not the most efficient nor numerically stable
...
For this simulation, the estimated prior is somewhat different from the
simulation parameters ($\sigma = 2$, $\nu = 5$), with noticeably lighter tails,
but it is certainly not outside the range of variation we might expect in
our estimates:
