...
posterior means $\mathbb{E}(\theta_i \mid x_i, s_i, \hat{g})$ and other 
...
specifying the prior. This estimation is typically done by maximizing the (log)
likelihood. There are many possible ways to perform this maximization; for
an overview of the dozens of R packages available for solving optimization
...
The `ebnm` package includes an `optmethod` argument that allows the user to 
choose among three general-purpose optimization methods when estimating 
parametric priors (e.g., normal or point-Laplace):
a Newton-type algorithm implemented in the built-in `nlm()` function; the 
L-BFGS-B algorithm, via the built-in `optim()` function; and the trust region 
...
greater than zero). To avoid numerical issues, we will set sensible lower
and upper bounds for both parameters (namely, 
$\min_i s_i / 10 \le \sigma \le \max_i x_i$ and $1 \le \nu \le 1000$).
Our optimization function will take initial parameter estimates as input and
return optimal values for the parameters of interest:
...
optimization function. Indeed, the gradient is difficult to compute, so
we allow `optim()` to estimate it using numerical methods. If, however, an
...
using **numDeriv** with a more efficient function):
