...
included in **ebnm**.) It is formally defined next.
...
For background and notation, please see the [ebnm paper][ebnm-paper].
...
scaled-*t* prior is to define a data structure for the priors in our
prior family. In some cases, an existing data structure can be
used. For example, `ebnm_normal()`, `ebnm_point_normal()`,
...
The other key computation required to fit the EBNM model is the
computation of various statistics (means, variances, etc.) from the
posterior distribution of the "true means" given the prior we
estimated. From Bayes' rule, the posterior distribution for the *i*th
unknown mean is
