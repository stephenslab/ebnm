...
$$
\thetas_i \sim 0.8\delta_2 + 0.2 N(2,1)
$$
...
$\hat{u}_i = E[\theta_i \,|\, x_i]$, which can be extracted with
`coef()`:
...
These "shrunken" estimates are better for $\theta_i$ values near 2, but
worse for ones far away from 2. Still, the shrunken estimates improve
...
means near 2 to be improved, while the estimates of the means far from 2
