---
title: "ebnm"
author: "Jason Willwerscheid, Peter Carbonetto, and Matthew Stephens"
date: "10/10/2023"
output: 
...
\begin{equation}
x_i \overset{\text{ind.}}{\sim} \mathcal{N}(\theta_i, s_i^2), (\#eq:nmmodel)
\end{equation}
...
\begin{equation}
\theta_i \overset{\text{ind.}}{\sim} g \in \mathcal{G}, (\#eq:nmprior)
\end{equation}
...
The EB approach
therefore involves fitting the model
\@ref(eq:nmmodel)--\@ref(eq:nmprior) by first using all of the
...
the EBNM problem using a wide variety of prior families. For some prior
...
robustness. The end result is a comprehensive and user-friendly
package for solving the EBNM problem. In this vignette, we demonstrate usage of
the `ebnm` package in a real application where normal means models have been used:
an analysis of weighted on-base averages for Major League Baseball players.
...
\begin{equation} 
\mathbf{z}^{(i)} \sim \text{Multinomial}(n_i, {\boldsymbol\pi}^{(i)}), (\#eq:wobamultinom)
\end{equation}
where ${\boldsymbol\pi}^{(i)} = (\pi_1, \ldots, \pi_7^{(i)})$ is the vector of
...
\end{equation}
where $\hat{\mathbf\Sigma}^{(i)}$ is the estimate of the covariance matrix
for the multinomial model \@ref(eq:wobamultinom) obtained by
...
\sum_{j=1}^N \mathbf{z}^{(i)}/ \sum_{j=1}^N n_i$, where $N$ is the
...
Indeed, this plot illustrates the ability of the
...
perform EBNM analyses with different types of priors, then compare
results across these different prior choices. Each prior family makes
...
numerical rounding error) in only 5 iterations. In some cases,
...
among the entire nonparametric family $\mathcal{G}$ (that is, provided
that the "true" prior is $g \in \mathcal{G}$; see
...
To address this issue, `deconvolveR` package [@NarasimhanEfron]
...
Indeed, the estimated prior $\hat{g}$ is much "smoother" than the NPMLE prior. 
