...
posterior quantiles and credible intervals, defined as the narrowest 
*continuous* intervals 
covering $(1 - \alpha)\%$ of the posterior, where $1 - \alpha$ is the 
...
estimates. For illustration, we calculate 80% credible intervals using 
10% and 90% quantiles (note that this is not the same as using the
`confint()` method with `level = 0.8`, since `confint()` returns the
*narrowest* continuous intervals covering 80% of the posterior):
...
not seem appropriate for the present analysis:
