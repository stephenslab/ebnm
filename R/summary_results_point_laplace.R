...
summary_results_point_laplace = function(x, s, w, a) {
  ...
  posterior_mean <- wpost * (l * my_etruncnorm(0, Inf, x - s^2 * a, s)
                             + (1 - l) * my_etruncnorm(-Inf, 0, x + s^2 * a, s))
  posterior_mean2 <- wpost * (l * my_e2truncnorm(0, Inf, x - s^2 * a, s)
                              + (1 - l) * my_e2truncnorm(-Inf, 0, x + s^2 * a, s))
