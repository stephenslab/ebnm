...
#'                               fix_pi0, fix_a, fix_mu,
#'                               optmethod, use_grad, use_hess) {
#'   # If everything is fixed, then just calculate the log likelihood.
#'   if (fix_pi0 && fix_a && fix_mu) {
#'     retlist <- g
#'     retlist$val <- loglik_point_laplace(x, s, 1 - g$pi0, g$a, g$mu)
#'     return(retlist)
#'   }
#'
#'   if (!fix_mu && any(s == 0)) {
...
#'
#'       optres <- do.call(optimize, c(list(f = pl_nllik), fn_params,
